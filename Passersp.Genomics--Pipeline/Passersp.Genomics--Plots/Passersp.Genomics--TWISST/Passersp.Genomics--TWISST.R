...
for (k in 1:length(Wlistgz)){
  ...
  col_prefix <- ifelse(grepl("WithMeerkerk", Wlistgz[k]), "Meerkerk_01",
                ifelse(grepl("WithY150239", Wlistgz[k]), "Focal Ind.", "Error"))
  ...
  Wlgz[[k]]$Phylo <- ifelse(grepl("WithMeerkerk", Wlistgz[k]), "Meerkerk_01",
                     ifelse(grepl("WithY150239", Wlistgz[k]), "Focal Ind.", "Error"))}
...
Meerkerk_df_WeightsDF <- dplyr::select(Meerkerk_df_WeightsDF, "Meerkerk_01 ~ House", "Meerkerk_01 ~ Spanish")
...
fulldf$PercY150239 <- as.numeric(fulldf$"Focal Ind. ~ Spanish") / (as.numeric(fulldf$"Focal Ind. ~ House") + as.numeric(fulldf$"Focal Ind. ~ Spanish")) * 100
fulldf$PercMeerkerk <- as.numeric(fulldf$"Meerkerk_01 ~ Spanish") / (as.numeric(fulldf$"Meerkerk_01 ~ House") + as.numeric(fulldf$"Meerkerk_01 ~ Spanish")) * 100
fulldf$Delta <- (fulldf$PercY150239 - fulldf$PercMeerkerk) / 100


# Reorders columns ~
WeightsDF <- dplyr::select(fulldf, "Phylo", "CHR", "TotalWeight", "Focal Ind. ~ House", "Focal Ind. ~ Spanish", "Meerkerk_01 ~ House", "Meerkerk_01 ~ Spanish", "PercY150239", "PercMeerkerk", "Delta")
...
fulldfUp <- gather(fulldf, Estimation, Value, "Focal Ind. ~ House", "Focal Ind. ~ Spanish", "Meerkerk_01 ~ House", "Meerkerk_01 ~ Spanish", "Delta")
...
Y150239_Plot <- 
ggplot() +
 geom_area(data = subset_df %>% filter(Estimation %in% c("Focal Ind. ~ House", "Focal Ind. ~ Spanish")),
 ...
 theme(panel.background = element_rect(fill = "#ffffff"),
       ...
       legend.box.margin = margin(t = 30, b = 30, r = 0, l = 0),
       legend.key = element_rect(fill = NA),
       legend.background = element_blank()) +
 guides(colour = guide_legend(title = "Topologies", title.theme = element_text(family = "Optima", size = 38, face = "bold"),
                              label.theme = element_text(family = "Optima", size = 34), override.aes = list(linewidth = 1.75, linetype = 1)),
        fill = guide_legend(title = "Topologies", title.theme = element_text(family = "Optima", size = 38, face = "bold"),
                            label.theme = element_text(family = "Optima", size = 34), override.aes = list(linewidth = .3, linetype = 1)))
  
    
# Create Meerkerk plot ~
Meerkerk_Plot <- ggplot() +
  geom_area(data = subset_df %>% filter(Estimation %in% c("Meerkerk_01 ~ House", "Meerkerk_01 ~ Spanish")),
            aes(x = as.numeric(Mid), y = as.numeric(Value), fill = Estimation, group = Estimation),
            position = "fill", colour = "#000000", alpha = .3, linetype = 1, linewidth = .2) +
  scale_x_continuous("Genomic Position",
                     breaks = breaks_and_labels$breaks,
...
# Save the panel ~
#ggsave(paste("Passersp.Genomics--TWISST_SW150_FillArea_", x, ".pdf", sep = ""), plot = Panel_Plot,
#       device = cairo_pdf, limitsize = FALSE, width = 40, height = 25, scale = 1, dpi = 600)}
#ggsave(paste("Passersp.Genomics--TWISST_FillArea_", x, ".jpeg", sep = ""), plot = Panel_Plot,
...
# Sets Gene Universe ~
head(GenesUniverse) <- (unique(GOTerms$Gene_ID))
...
write.table(FocalGenes_list[[cat]], file = paste0("Passersp.Genomics--GOAnalysis--FocalGenes_", cat, ".csv"),
...
# Saves the list ~ 
saveRDS(GOTermsOrtho_Edited_list, file = "../Passersp.Genomics--GOAnalysis/GOTermsOrtho_Edited_list.rds")
  
  
# Saves GOTermsOrtho ~
write.table(GOTermsOrtho_Edited_list[[cat]], file = paste0("Passersp.Genomics--GOAnalysis--GOTermsOrtho_Edited_", cat, ".csv"),
            sep = "\t", quote = FALSE, row.names = FALSE)
...
GO_Enrich_list[[cat]] %>% arrange(Pvalue) %>%
                          write.csv(file = paste0("Passersp.Genomics--GOAnalysis_", cat, ".csv"))
...
# Saves list  ~
write.table(GOEnrich_Top50_combined, file = "Passersp.Genomics--GOAnalysis_Top50_Combined.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
FocalGenes_list_outliers <- FocalGenes_list[["outliers"]]
saveRDS(FocalGenes_list_outliers, file = "FocalGenes_outliers.rds")


# Saves list  ~
write.table(GOEnrich_Top50_combined, file = "Passersp.Genomics--TWISSTGOAnalysis--FocalGenes_list_outliers.txt", sep = "\t", quote = FALSE, row.names = FALSE)


ListSize <- length(FocalGenes_list_outliers$Gene_ID)
BackgroundSize <- length(GenesUniverse)


GOEnrich_Top50_combined_AllOutliers <- GOEnrich_Top50_combined %>%
                                       filter(Category == "outliers") %>%
                                       mutate(Count = as.numeric(Count),
                                              Size = as.numeric(Size))



GOEnrich_Top50_combined_AllOutliers$FoldEnrichment <- (GOEnrich_Top50_combined_AllOutliers$Count / ListSize) / (GOEnrich_Top50_combined_AllOutliers$Size / BackgroundSize)


# Apply the function to the pattern column ~
GOEnrich_Top50_combined_AllOutliers <- GOEnrich_Top50_combined_AllOutliers %>%
...
# Reorders Population ~
GOEnrich_Top50_combined_AllOutliers$Size_cat <- factor(GOEnrich_Top50_combined_AllOutliers$Size_cat, ordered = TRUE,
                                                       levels = c("< 50",
                                                                  "< 100",
                                                                  "< 250",
                                                                  "< 500",
...
# Get top 50 enriched terms
GOEnrich_Top50_combined_AllOutliers_Abridged <- GOEnrich_Top50_combined_AllOutliers %>%
                                                arrange(Pvalue) %>%
                                                head(10)


# Defines color palette and breaks ~
color_palette <- c("#d73027", "#4575b4", "#EAEDE9")  
nHalf <- 10
Min <- 0
Max <- .005
Thresh <- 0
...
max(GOEnrich_Top50_combined_AllOutliers_Abridged$Pvalue)


# Creates AIMs GO Analysis plot ~
GOPlot <- ggplot(GOEnrich_Top50_combined_AllOutliers_Abridged, aes(x = log1p(FoldEnrichment), y = Term)) +
  geom_segment(aes(x = 0, xend = log1p(FoldEnrichment), y = Term, yend = Term, color = Pvalue), linewidth = 12) +
  geom_point(aes(size = Size_cat), shape = 21, fill = "#ffffff", color = "#000000", stroke = .15) +
  scale_size_manual(values = c("< 50" = 7.5, "< 100" = 4.5, "< 250" = 10.5, "< 500" = 7.5, "< 750" = 9, "> 750" = 10.5)) +
  ...
  scale_color_gradientn(colors = rampcols,
                        limits = c(0, .005),
                        breaks = c(.001, .002, .003, .004),
                        labels = c(0.001, 0.002, 0.003, 0.004)) +
...
ggsave(plot = GOPlot, "Passersp.Genomics--TWISSTGOAnalysis.pdf",
       device = cairo_pdf, limitsize = FALSE, width = 12, height = 8, dpi = 600)
ggsave(plot = GOPlot, "Passersp.Genomics--TWISSTGOAnalysis.jpeg",
       limitsize = FALSE, width = 12, height = 8, dpi = 600)
...
# Saves plot ~
ggsave(GOAnalysis_Plot, file = "Passersp.Genomics--TWISST_GOAnalysis.pdf",
       device = cairo_pdf, limitsize = FALSE, width = 12, height = 15, scale = 1, dpi = 600)
ggsave(GOAnalysis_Plot, file = "Passersp.Genomics--TWISST_GOAnalysis.jpeg",
