...
# Function to generate ordered pairwise permutations for a given set of individuals
generate_ordered_permutations <- function(individuals, k) {
  # Ensure the order is preserved by looping through each individual in order
...
all_permutations <- do.call(rbind, lapply(seq_along(corres), function(i) {
  individuals <- corres[[i]]$Sample_ID_1  # Get unique individuals
  k <- corres[[i]]$K[1]                  # Get the K value
  generate_ordered_permutations(individuals, k)
}))

# Add the "Order" column starting from 1 for each first occurrence of Sample_ID_1 per K
all_permutations <- all_permutations %>%
  group_by(K) %>%
  mutate(Order = match(Sample_ID_1, unique(Sample_ID_1))) %>%
  ungroup()  # Ungroup to avoid any unintended behavior in further operations


# Ensure fulldf$K is treated as a character for proper matching
fulldf$K <- as.character(fulldf$K)

# Function to reorder rows of fulldf based on all_permutations
reorder_fulldf <- function(df, permutations) {
  # Add an ordering column to permutations for consistent sorting
  permutations$order <- seq_len(nrow(permutations))
  
  # Merge fulldf with permutations to align rows
  merged <- merge(permutations, df, by = c("Sample_ID_1", "Sample_ID_2", "K"), all.x = TRUE)
  
  # Sort merged data by the original order in permutations
  reordered <- merged[order(merged$order), ]
  
  # Drop helper columns and return, keeping the Order column
  reordered$order <- NULL
  return(reordered)
}

# Split fulldf and all_permutations by K
split_fulldf <- split(fulldf, fulldf$K)
split_permutations <- split(all_permutations, all_permutations$K)

# Apply the reordering function to each subset of fulldf
fulldfUp <- do.call(rbind, lapply(names(split_fulldf), function(k) {
  ...
  # Add the Order column from permutations to the resulting dataframe
  reordered_df$Order <- split_permutations[[k]]$Order
  return(reordered_df)
}))


# Set factor levels for Sample_ID_1 and Sample_ID_2 per K
fulldfUp <- fulldfUp %>%
  group_by(K) %>%
  mutate(
    Sample_ID_1 = factor(Sample_ID_1, levels = unique(Sample_ID_1)),
    Sample_ID_2 = factor(Sample_ID_2, levels = unique(Sample_ID_1)) # Same order as Sample_ID_1
  ) %>%
  ungroup()
...
# Define a plotting function for a specific K
plot_for_K <- function(k_value, facet_type = "default", show_x_labels = FALSE, is_last_plot = FALSE) {
  ...
  ggplot(subset_data, aes(x = Sample_ID_1_ordered, y = Sample_ID_2_ordered, fill = as.numeric(Value))) +
    # Tiles for individual and diagonal cells (with borders)
    geom_tile(linewidth = 0.15,  colour = "#000000") +
    ...
    theme(
      ...
      legend.position = ifelse(is_last_plot, "right", "none"),  # Only show legend for the last plot
...
# Generate the plots for each K (without legend on all except last plot)
unique_K_values <- rev(unique(fulldfUp$K))  # Reverse the order of the K values
