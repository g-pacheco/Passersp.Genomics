plotCorRes <- function(cor_mat, pop=NULL, ord=NULL, superpop=NULL,
                       ...
                       adjlabsuperpop=0.16, cex.lab.2 = 1.5, save_objects=FALSE, save_dir="."){
  ...
  for (i1 in 1:(length(unique(pop)))) {
    for (i2 in 1:(length(unique(pop)))) {
      p1 <- unique(pop)[i1]
      p2 <- unique(pop)[i2]
      mean_cors[i1, i2] <- mean(cor_mat[which(pop == p1), which(pop == p2)][!is.na(cor_mat[which(pop == p1), which(pop == p2)])])
    }
  }
  
  if (save_objects) {
    # Save cor_mat and mean_cors as data frames
    cor_mat_df <- as.data.frame(cor_mat)
    mean_cors_df <- as.data.frame(mean_cors)
    
    write.table(cor_mat_df, file=file.path(save_dir, "cor_mat.csv"), row.names = TRUE, sep = "\t", quote = FALSE)
    write.table(mean_cors_df, file=file.path(save_dir, "mean_cors.csv"), row.names = TRUE, sep = "\t", quote = FALSE)
  }
...
orderInds <- function(q=NULL, pop=NULL, popord=NULL){
  ...
  # recommended is to use pop, then if q is given it will order within pop by admixture proportion. poporder allows to pre-specify order of populations
